...
export default function CollectionForm({ _id }: { _id?: string }) {
  // 1. set state
        ...
        toast({
          ...
          action: (
            <ToastAction altText={`Go to ${data.data.name}`}>
              <Link href={`/admin/collections/${data.data._id}`}>
    ...
    return await axios
      ...
      .then((response) => {
        ...
        toast({
          ...
          action: (
            <ToastAction altText={`Go to ${data.data.name}`}>
              <Link href={`/admin/collections`}>Go to List</Link>
            </ToastAction>
          ),
        });
      })
  ...
  const form = useForm<z.infer<typeof collectionValidationSchema>>({
    resolver: zodResolver(collectionValidationSchema),
    defaultValues: collection
      ? collection
      : {
          name: "",
          description: "A simple description should be overwhrite",
          slug: "",
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      setLoading(true);
      await axios
        .get(process.env.NEXT_PUBLIC_API_URL + "/api/admin/collections", {
          params: { _id: _id },
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
  ...
  const onSubmit = async (
    values: z.infer<typeof collectionValidationSchema>
  ) => {
    const data = {
      name: values.name,
      slug: values.slug,
